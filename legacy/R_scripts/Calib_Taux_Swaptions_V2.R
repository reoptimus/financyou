###############
# fct: Calib_Taux_Swaptions_M1 avec a fixé
###############
...
# pas_t_calib = le pas de temps en année (ex: 0.5 pour 6mois)
###########################################

Calib_Taux_Swaptions_M1 <- function(Prix_swaptions,reptravail,Param_EIOPA,nom_sortie,pas_t_calib)
  {
  library(xlsx)
  library(plot3D)
  library(plot3Drgl)
...
pas_t_calib<-0.025
reptravail<-'\\\\intra/partages/UA2771_Data/3_ST_top-down/3.2_En_Cours/6. ESG_V2/2. Organisation scripts'
Prix_swaptions<-"Prix_swaptions_bloomberg.Rda" #nom variable: PS_Norm_mat
Param_EIOPA<-"EIOPA_avril_2018_FRANCE.xlsx"
nom_sortie<-"a_sigma"
##################

Mat_EIOPA<-read.xlsx(file=paste(reptravail,"/R4 CALIBRAGE/R4 IN/",Param_EIOPA, sep=""),1, header=FALSE)

#chargement de la courbe des taux (EIOPA) et recalcul des courbes f0t et P0t
source(paste(reptravail,"/R4 CALIBRAGE/R4 FCT/","Calib_Tauxf0_V2.R",sep=""))
# variables internes (ecrasent la derniere serie de P0t et f0t)
nom_sortie_P0t<-"Prix_P0t_interp"
nom_sortie_f0t<-"f0t_liss"
Calib_Tauxf0(Param_EIOPA,paste(reptravail,"/R4 CALIBRAGE",sep=""),pas_t_calib)
load(file=paste(reptravail,"/R4 CALIBRAGE/R4 OUT/",nom_sortie_P0t,".Rda",sep=""))
load(file=paste(reptravail,"/R4 CALIBRAGE/R4 OUT/",nom_sortie_f0t,".Rda",sep=""))
####################
...
a<-c(0.005,0.01,0.02)
deb_sigma<-0.02 # en %/100
pas_sigma<-deb_sigma/((nb_pas_opt-1)/2)
...
for (m in 1:nb_bcl) { #nombre d'iteration decidé à l'avance
    ...
    MTauxswaptions<-array(0,dim=c(length(sigma),length(a),nb_swaptions))
      ...
      for (i in 1:nb_swaptions) {
        # matrice des tirages aleatoires pour le calcul par methode MonteCarlo adaptée au temps de chaque swaption
        N<-1000 #dim(Mat_resid) [2]
        nb_pas<-(S[i]/pas_t_calib+1)
        ...
        for (j in 1:length(sigma)){
          ...
          MTauxswaptions[j,k,i]<-uniroot(Prix_swaption_Normal_Uniroot,Ta=Maturite[i],Tb=S[i],n=t_jambe[i],K=Strike[i],P0t=P0t_interp,prixsw=Prixswaptions,pas_P0t=pas_t_calib, interval=c(-10,10),tol=0.0001,extendInt="yes")$root
            }
        }
...
a<-a_min
a<-a/((pas_t_calib)^0.5) #remise du Sd sur une base annuelle
save(a,sigma,pas_t_calib, file=paste(reptravail,"/R4 CALIBRAGE/R4 OUT/",nom_sortie,".Rda",sep=""))
#load(file=paste(reptravail,"/R4 CALIBRAGE/R4 OUT/",nom_sortie,".Rda",sep=""))

#efface les variables utilisées pour les calculs
rm(list=c("a_min","MTauxswaptions","erreur","erreur_tot","r0","obj","Msigma","Ma","gap","deb_sigma","deb_a","Ponderation3D","nb_swaptions","nb_pas_opt","i","j","k","gap_a","gap_sigma","a_min","sigma_min","pas_a","pas_sigma"))
#rm(list=ls())
gc()
