...
ZC <- function(param_calib,Taux_ss_risque,nom_sortie,dur,pas,reptravail,sc)
{
    #################################### A effacer
  #Paramètres figés pour démonstration
  #param_calib<-"Param_Rt.Rda"
  ...
  load(file=paste(reptravail,"/R2 CUBE/R2 IN/a_sigma.Rda", sep="")) #parametre calibre pour les taux H&W
  k <- a
  volHW <- sigma
  #f0 var globale
  ...
  # mise en forme .Rda
  #save(k,volHW,f0,ZC_t,file=paste(reptravail,"/R2 IN/Param_Rt.Rda",sep=""))
  ###################################
  # Chargement des paramètres calibrés
  # contient k,volHW,f0 (vecteur dim=120)
  #load(paste(reptravail,"/R2 CUBE/R2 IN/",param_calib, sep="") ) #parametres de calibrage des différents processus
  ...
  f03D<-array(rep(f0[t_liste],d_tot*N),dim=c(Tf,d_tot,N))
