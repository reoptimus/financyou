...
#
# crée le: 10/12/2019 par Sébastien Gallet
# modifiée le: 07/01/2020
#
# Version : 2 (HW marting avec loyers fixes et infla loyers flat)
#
# variables d entree#######################
...
immo <- function(param_calib,Param_EIOPA,sc,Mat_resid_name,nom_sortie,delta_t,reptravail)
  ...
  #chargement du de la tranche Rt
  load(paste(reptravail,"/R1 MEFCUBE/R1 IN/","Tr-Rt.Rda", sep="") )
  load(paste(reptravail,"/R1 MEFCUBE/R1 IN/","rt.Rda", sep="") )
  ...
  Infl<-log(1-0.00)*delta_t # % d'inflation sur les loyers immobiliers hors infl monÃ©taire
  L0_P0<-log(1+0.01)*delta_t #% annuel en taux continu
  ...
  for (i in 1:(Tf)) {
    ...
    kimmo<-f0t_liss[i]#r0_2*(a2+rt[,i])/a2#r0_2*(a2+rt[,i])
    ...
    #RdtImmoL[,i]<-log(1+L0_P0*exp(-RdtImmoCumul+i*Infl)) #loyer V1
    RdtImmoL[,i]<-L0_P0+i*Infl  #loyer V2
